...
import PartyKit.Server (Connection, ConnectionContext, ExecutionContext, Lobby, PartyServer, Request, create)
...
recalculateAndBroadcast room = do
  userOps :: Map String (Tuple Int Int) <- Room.get (Room.storage room) "ops" <#> fromMaybe Map.empty
  ...
  case msg of
    Left str -> do
      case readJSON str of
        ...
        Right message -> do
          userOps :: Map String (Tuple Int Int) <- Room.get s "ops" <#> (fromMaybe Map.empty)
          let
